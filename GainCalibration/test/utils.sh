...
submit_to_queue(){
 if [ `working_on_lxplus` -eq 1 ]; then
   bsub -q $q_lxplus -J $1 -eo $2 < $3
     ...
     if [ `working_on_T3` -eq 1 ]; 
	 then qsub -q long.q -j y -N $1 -o $2 $3
     else
...
set_specifics(){
  ...
  else if [ `is_on_T3 $1` -eq 1 ]; then set_T3_specifics;
...
is_on_T3(){
    if [ `echo $1 | grep -e "t3" | wc -l` -eq 1 ]; then echo 1
    else echo 0
    fi
}
...
working_on_T3(){
    if [ `uname -a | grep t3ui | wc -l` -gt 0 ]; then echo 1
    else echo 0
    fi
}
